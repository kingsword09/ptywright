...
test("MCP server smoke test", async () => {
  ...
  // These are now hidden/removed
  expect(list.tools.some((t) => t.name === "send_mouse")).toBe(false);
  expect(list.tools.some((t) => t.name === "snapshot_grid")).toBe(false);
  expect(list.tools.some((t) => t.name === "snapshot_cast")).toBe(false);
  ...
  // Removed hidden snapshot_cast usage
