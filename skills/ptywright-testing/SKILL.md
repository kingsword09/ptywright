...
- MCP: call `ptywright_run_all_scripts` with no args (defaults) or set `dir`/`artifactsRoot`.
...
- MCP: `ptywright_run_script(scriptPath=...)`
...
- Start: `ptywright_start_script_recording(name=...)`
- Drive the app via normal tools (`ptywright_launch_session`, `ptywright_press_key`, `ptywright_send_text`, `ptywright_wait_for_*`).
- Mark checkpoints: `ptywright_mark(label=...)` (creates golden snapshots)
- Stop + write: `ptywright_stop_script_recording(recordingId=..., writeFiles=true)`

## Agent Hygiene

- Prefer `run.summary.json` for reasoning; avoid pasting long `report.html` into chat.
- Keep MCP output small: `ptywright_run_all_scripts(includeEntries="failures", maxEntries=...)`.
